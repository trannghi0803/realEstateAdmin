...
export default function FormRegister(props: IProps, states: IStateProps) {
    ...
    const submitRegister = async (event:any )=>{
        ...
        if(checkValidate){
            ...
            const create = await new UserService().register(data);
            if(!create?.value?.responseException?.isError){
                Helpers.showAlert(Strings.Home.REGISTER_SUCCESS , "success");
                props.onCreateSuccess(false);
            }else{
                const mes = create?.value?.responseException?.validationErrors;
                const stringMes: any = []
                mes.forEach((element: any) => {
                    stringMes.push(element.message)
                });
                Helpers.showAlert(stringMes.join(' ') , "error");
            }
        }   
    }
